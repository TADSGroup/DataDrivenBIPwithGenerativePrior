...
class Trainer():
    ...
    # iteratiors for training the discriminator
    def _critic_train_iteration(self, data_in,data_gen,log=True):
        """
        Data is the data to condition on combined with the output of Generator
        """
        data_cat_in = data_in
        generated_data = self.sample_generator(data_cat_in)
        combined=data_gen 

        generated_data_combined = generated_data
        combined = Variable(combined)

        combined = combined.to(self.device)

        combined = torch.cat((0.0*torch.randn((combined.shape[0], 1), dtype=torch.float32).to(self.device),combined),axis=1)
        generated_data_combined = torch.cat((0.0*torch.randn((generated_data_combined.shape[0], 1), dtype=torch.float32).to(self.device),generated_data_combined),axis=1)
    ...
    # iterations for training the generator
    def _generator_train_iteration(self, data_in,data_gen, data_loader, log = True):
        ...
        data_cat_in = data_in
        latent_samples,generated_data = self.sample_generator(data_cat_in,return_input=True)
        generated_data_combined = generated_data
        generated_data_combined = torch.cat((0.0*torch.randn((generated_data_combined.shape[0], 1), dtype=torch.float32).to(self.device),generated_data_combined),axis=1)
        d_generated = self.D(generated_data_combined)
    ...
    def _train_epoch(self, data_loader,update_generator = True):
        ...
        """
        for i, (data_in,data_gen) in enumerate(data_loader):
            whether_to_log = (self.num_steps%(self.wandb_log_every*self.critic_iterations))==0
            data_gen = data_gen.to(self.device)
            data_in = data_in.to(self.device)
            self._critic_train_iteration(data_in, data_gen,log=whether_to_log)
            if (self.num_steps % self.critic_iterations == 0) and (update_generator is True):
                self._generator_train_iteration(data_in, data_gen,data_loader, log=whether_to_log)
            # update the generator every 20 iterations of training the discriminator
            self.num_steps += 1

            
    # training process, includes wandb data printing
    ...
    def sample_generator(self, data_in,return_input=False):
        generator_input = data_in 
