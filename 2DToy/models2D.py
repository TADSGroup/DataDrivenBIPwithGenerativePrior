...
class Generator(nn.Module):
    def __init__(
        ...
        layer_dimensions = [condition_dim+sample_dim] + hidden_layer_sizes + [2]
    ...
    def forward(self, input_data_combined):
        ...
        # Apply residual output for sequential training
        return input_data_combined+self.network(input_data_combined) 

    def sample_latent(self, num_samples):
        return torch.randn((num_samples, self.sample_dim))
    
    def sample_concat_latent(self,data_cond):
        print(data_cond.shape[0])
        return torch.cat(
            (
                data_cond,
                self.sample_latent.to(data_cond.device)
                )
                ,
                axis=1
                )
